...
The README states half the 20 cases are legitimate. The agent calls **7/20 legitimate (35%)**, 6/20 fraud (30%) and 7/20 uncertain (35%).

The agent does not call fraud on more than half (30%), so it is not blocking everything. 5/20 final recommendations include BLOCK_CARD and 3/20 file a SAR.
...
| 2 | HHG-015 | 2016-11-17 19:03:36 | GC-HHG-015 | 9 | 1 | 0 | 3 | 2 | MATCHES_PATTERN 1 |
...
| 7 | HHG-019 | 2016-12-01 22:28:53 | GC-HHG-019 | 8 | 1 | 0 | 4 | 2 | MATCHES_PATTERN 1 |
| 8 | HHG-010 | 2016-12-02 18:18:27 | GC-HHG-010 | 9 | 1 | 1 | 3 | 3 | MATCHES_PATTERN 1 |
| 9 | HHG-020 | 2016-12-03 12:04:26 | GC-HHG-020 | 10 | 1 | 1 | 2 | 2 | MATCHES_PATTERN 1 |
| 10 | HHG-001 | 2016-12-05 01:55:28 | GC-HHG-001 | 5 | 1 | 0 | 4 | 2 | MATCHES_PATTERN 1 |
| 11 | HHG-007 | 2016-12-05 03:46:14 | GC-HHG-007 | 5 | 1 | 0 | 8 | 2 | MATCHES_PATTERN 1 |
...
| 14 | HHG-003 | 2016-12-10 15:01:21 | GC-HHG-003 | 8 | 1 | 1 | 6 | 3 | MATCHES_PATTERN 1 |
| 15 | HHG-016 | 2016-12-12 01:39:08 | GC-HHG-016 | 8 | 1 | 1 | 4 | 2 | MATCHES_PATTERN 1 |
| 16 | HHG-012 | 2016-12-18 05:00:31 | GC-HHG-012 | 6 | 1 | 0 | 2 | 2 | MATCHES_PATTERN 1 |
| 17 | HHG-008 | 2016-12-20 03:08:56 | GC-HHG-008 | 8 | 1 | 1 | 8 | 3 | MATCHES_PATTERN 1 |
| 18 | HHG-009 | 2016-12-28 17:10:53 | GC-HHG-009 | 8 | 1 | 0 | 0 | 2 | MATCHES_PATTERN 1 |
...
| Fraud calls: TP / FP / FN | 9 / 4 / 16 |
| **Precision on fraud calls** | 69% |
| **Recall on fraud calls** (uncertain counts as a miss) | 36% |
| Recall if uncertain on a confirmed case counts as caught | 92% |
| Fraud-call rate overall | 26% |
| **Legitimate verdict on cleared cases** | 48% |
| Verdicts on cleared | {"fraud": 4, "legitimate": 12, "uncertain": 9} |
| Verdicts on confirmed | {"uncertain": 14, "legitimate": 2, "fraud": 9} |
| **Pattern accuracy, confirmed cases** | 48% |
| Pattern accuracy, all cases (none on cleared counts) | 48% |
...
| card_not_present_fraud | 4 | 0 | 0 |
| card_not_present_new_device | 4 | 3 | 3 |
| card_testing | 4 | 0 | 1 |
| out_of_region_use | 4 | 1 | 2 |
| undocumented | 5 | 5 | 5 |

Most common pattern confusions (truth -> agent): card_testing -> card_not_present_fraud (3); card_not_present_fraud -> card_not_present_new_device (3); account_takeover -> out_of_region_use (2); card_not_present_fraud -> none (1); card_not_present_new_device -> account_takeover (1); out_of_region_use -> account_takeover (1).

What the backtest says:

- **The verdict does not separate cleared from confirmed cases.** Mean fraud probability is 0.35 on cleared cases and 0.61 on confirmed ones, and the verdict mix is almost the same in both groups. Precision on fraud calls (69%) is what a coin would give on a balanced sample.
- **Legitimate is almost never called on a cleared case (48%).** Most cleared cases land in `uncertain`, and several get `fraud` with probability near 0.9, usually as card_not_present_new_device. This matches the low legitimate rate on the 20 benchmark cases and is the main thing to fix: the new-device signal is scored as strong even though the README says people buy new phones.
- **Recall is low if `uncertain` counts as a miss (36%) and high if it does not (92%).** The agent rarely lets a confirmed case go (one `legitimate`), but it defers most of them.
...
|---|---|---|---|---|---|---|---|---|
| HHG-017 | 2016-11-12 00:46:24 | legitimate | 0.07 | none | $0.00 | no | ALLOW_TRANSACTION (auto), CLOSE_NO_FRAUD (auto) | unchanged |
| HHG-015 | 2016-11-17 19:03:36 | legitimate | 0.05 | none | $0.00 | no | CREATE_CASE (auto), ESCALATE_TO_ANALYST (auto), STEP_UP_AUTH (auto) | CREATE_CASE (auto), CLOSE_NO_FRAUD (auto) |
...
| HHG-014 | 2016-11-22 20:11:00 | fraud | 0.88 | undocumented | $187.33 | yes | CREATE_CASE (auto), FILE_REPORT (L2), MONITOR_CONNECTED_CARDS (auto), ESCALATE_TO_ANALYST (auto) | unchanged |
| HHG-002 | 2016-11-22 23:27:07 | uncertain | 0.41 | card_not_present_fraud | $292.36 | no | STEP_UP_AUTH (auto), VERIFY_WITH_CUSTOMER (auto), CREATE_CASE (auto) | CREATE_CASE (auto), MONITOR_CARD (auto) |
| HHG-018 | 2016-11-27 14:41:26 | uncertain | 0.45 | account_takeover | $39.08 | no | STEP_UP_AUTH (auto), VERIFY_WITH_CUSTOMER (auto), CREATE_CASE (auto), ESCALATE_TO_ANALYST (auto) | CREATE_CASE (auto), MONITOR_CARD (auto), ESCALATE_TO_ANALYST (auto) |
| HHG-019 | 2016-12-01 22:28:53 | legitimate | 0.04 | none | $0.00 | no | ALLOW_TRANSACTION (auto), CLOSE_NO_FRAUD (auto) | unchanged |
| HHG-010 | 2016-12-02 18:18:27 | fraud | 0.90 | card_not_present_new_device | $1,000.03 | yes | CREATE_CASE (auto), ESCALATE_TO_ANALYST (auto), STEP_UP_AUTH (auto) | BLOCK_CARD (L1), CREATE_CASE (auto), FILE_REPORT (L2) |
| HHG-020 | 2016-12-03 12:04:26 | fraud | 0.85 | card_not_present_new_device | $125.08 | no | CREATE_CASE (auto), MONITOR_CARD (auto), STEP_UP_AUTH (auto) | BLOCK_CARD (L1), CREATE_CASE (auto) |
| HHG-001 | 2016-12-05 01:55:28 | legitimate | 0.18 | none | $0.00 | no | ALLOW_TRANSACTION (auto), CLOSE_NO_FRAUD (auto) | unchanged |
| HHG-007 | 2016-12-05 03:46:14 | legitimate | 0.23 | none | $0.00 | no | ALLOW_TRANSACTION (auto), CLOSE_NO_FRAUD (auto) | unchanged |
| HHG-005 | 2016-12-08 03:38:37 | uncertain | 0.37 | card_not_present_new_device | $100.07 | no | STEP_UP_AUTH (auto), VERIFY_WITH_CUSTOMER (auto), CREATE_CASE (auto) | DECLINE_TRANSACTION (L1), CREATE_CASE (auto), MONITOR_CARD (auto) |
| HHG-013 | 2016-12-09 05:39:29 | uncertain | 0.47 | account_takeover | $35.66 | no | STEP_UP_AUTH (auto), VERIFY_WITH_CUSTOMER (auto), CREATE_CASE (auto) | DECLINE_TRANSACTION (L1), CREATE_CASE (auto), MONITOR_CARD (auto) |
| HHG-003 | 2016-12-10 15:01:21 | uncertain | 0.35 | out_of_region_use | $49.00 | no | STEP_UP_AUTH (auto), VERIFY_WITH_CUSTOMER (auto), CREATE_CASE (auto), ESCALATE_TO_ANALYST (auto) | CREATE_CASE (auto), MONITOR_CARD (auto), ESCALATE_TO_ANALYST (auto) |
| HHG-016 | 2016-12-12 01:39:08 | uncertain | 0.56 | card_not_present_new_device | $59.67 | no | STEP_UP_AUTH (auto), VERIFY_WITH_CUSTOMER (auto), CREATE_CASE (auto) | CREATE_CASE (auto), MONITOR_CARD (auto) |
| HHG-012 | 2016-12-18 05:00:31 | legitimate | 0.14 | none | $0.00 | no | ALLOW_TRANSACTION (auto), CLOSE_NO_FRAUD (auto) | unchanged |
| HHG-008 | 2016-12-20 03:08:56 | uncertain | 0.41 | card_not_present_fraud | $55.68 | no | STEP_UP_AUTH (auto), VERIFY_WITH_CUSTOMER (auto), CREATE_CASE (auto), ESCALATE_TO_ANALYST (auto) | CREATE_CASE (auto), MONITOR_CARD (auto), ESCALATE_TO_ANALYST (auto) |
| HHG-009 | 2016-12-28 17:10:53 | legitimate | 0.02 | none | $0.00 | no | STEP_UP_AUTH (auto), VERIFY_WITH_CUSTOMER (auto), CREATE_CASE (auto) | CREATE_CASE (auto), CLOSE_NO_FRAUD (auto) |
